...
(function ($) {
    ...
    function linkToNodeUrl (page) {
        if (!Drupal.settings.tincanapi || !Drupal.settings.tincanapi.trackable)
            return false;

        if (Drupal.settings.tincanapi.aliases && Drupal.settings.tincanapi.aliases[page])
            page = Drupal.settings.tincanapi.aliases[page];

        if (Drupal.settings.tincanapi.trackable[page])
            return page;
    ...
    function trackMediaOnPage() {
        ...
        if (Drupal.settings.tincanapi && Drupal.settings.tincanapi.settings) {
            var trackable = Drupal.settings.tincanapi.settings.media_tracking_types;
    ...
    Drupal.behaviors.tincanapi = {
        attach: function () {
            ...
            var tracked = trackUrl(Drupal.settings.tincanapi.modalPage, {
                parent: parent
            });
